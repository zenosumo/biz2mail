...
def get_website_from_duckduckgo(vat, company_name):
    ...
    domain_extensions = ['.com', '.it', '.net', '.org']
    for ext in domain_extensions:
        query = f"{company_name} site:{ext}"
        url = f"https://duckduckgo.com/html/?q={query}"
        response = requests.get(url, headers=headers)
        log_details.append(f"Searching for {query}, status code: {response.status_code}")

        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            results = soup.find_all('a', class_='result__a', href=True)
            if results:
                log_details.append(f"[{company_name}]")
                log_details.append(f"Found {len(results)} results for {query}:")
                ...
                log_details.append(f"No result found for {query}")
        else:
            log_details.append(f"Error: Received status code {response.status_code} for query: {query}")
